...
unsigned int minimaxinit(int depth, boardstate brd, bool maximizingPlayer)
{
    ...
    if (maximizingPlayer)
    {
        ...
        unsigned int movelist[128]{0};   //maybe 128 works well
        brd.getmove(movelist);          //complete getmove
        std::sort(movelist, movelist + sizeof(movelist) / sizeof(movelist[0]), std::greater<int>());
 /*           [brd,alpha,beta](const unsigned int A, const unsigned int B)->bool {
                return (minimax(3,brd,A,true,alpha,beta) > minimax(3, brd, B,true, alpha, beta));
            }
        );  //does sort work if null elements*/
        if(movelist[0] == 0 && brd.ischecked(1)){  //checkmate
            std::cout<<"# white checkmated\n";
            return 0b0001000000000000;// BLACKWINS;
    ...
    else
    {
        ...
        unsigned int movelist[128]{0};

        brd.getmove(movelist);

        std::sort(movelist, movelist + sizeof(movelist) / sizeof(movelist[0]),std::greater<int>()
            /*[brd, alpha, beta](const unsigned int A, const unsigned int B)->bool {
                return (minimax(3, brd, A, false, alpha, beta) > minimax(3, brd, B, false, alpha, beta));
            }*/
        );

        if(movelist[0] == 0 && brd.ischecked(0)){  //checkmate
            std::cout<<"# black checkmated\n";
            return 0b0010000000000000;// WHITEWINS;
